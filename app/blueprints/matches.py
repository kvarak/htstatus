...
def has_enhanced_data_filter(match):
    ...
    try:
        ...
        return (
            getattr(match, "home_team_possession", None) is not None
            or getattr(match, "home_team_shots", None) is not None
            or getattr(match, "attendance", None) is not None
        )
    except Exception:
...
def matches():
    ...
    try:
        ...
        if "all_teams" not in session or not session["all_teams"]:
            ...
            return create_page(
                template="matches.html",
                error="No teams available. Please update your data first.",
                title="Matches",
            )
        ...
        if "all_team_names" not in session or not session["all_team_names"]:
            ...
            return create_page(
                template="matches.html",
                error="Team data incomplete. Please update your data.",
                title="Matches",
            )
        ...
        try:
            ...
            return create_page(
                template="matches.html",
                error="Team not found in your teams.",
                title="Matches",
            )
        ...
        try:
            ...
            return create_page(
                template="matches.html",
                error="Error fetching matches from database.",
                title="Matches",
            )
        ...
        for m in dbmatches:
            try:
                ...
                dprint(
                    2,
                    f"Error processing match {getattr(m, 'ht_id', 'unknown')}: {str(e)}",
                )
                continue
...
def formations():
    ...
    selected_formation = request.args.get(
    ...
    if request.method == "POST" and request.form.get("analyze"):
        ...
        for position_id in formation_positions:
            ...
            if player_ht_id:
                ...
                player = next(
                    (p for p in current_players_list if str(p.ht_id) == player_ht_id),
                    None,
                )
                if player:
                    player_assignments[position_id] = player

        formation_analysis = calculate_formation_effectiveness(
            selected_formation, player_assignments
        )
