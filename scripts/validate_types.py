    ...
    for interface_name, interface_content in matches:
        ...
        if interface_name in [
            ...
            for field_name, optional_marker, field_type in field_matches:
                ...
                fields[field_name] = {"type": field_type, "optional": is_optional}
...
def validate_type_sync(
    models: dict[str, dict[str, Any]], interfaces: dict[str, dict[str, Any]]
) -> list[str]:
    ...
    for model_name in models:
        ...
        for field_name, field_info in model_fields.items():
            if field_name not in interface_fields:
                errors.append(
                    f"❌ {model_name}.{field_name}: Missing in TypeScript interface"
                )
                continue
            ...
            if ts_type != expected_ts_type:
                errors.append(
                    f"❌ {model_name}.{field_name}: Type mismatch - SQLAlchemy '{sql_type}' -> expected '{expected_ts_type}', got '{ts_type}'"
                )
            ...
            if is_nullable != is_optional:
                ...
                errors.append(
                    f"⚠️  {model_name}.{field_name}: Nullability mismatch - SQLAlchemy {nullable_str}, TypeScript {optional_str}"
                )
        ...
        for field_name in interface_fields:
            if field_name not in model_fields:
                errors.append(
                    f"⚠️  {model_name}.{field_name}: Extra field in TypeScript interface (not in model)"
                )
    ...
    for interface_name in interfaces:
        if interface_name not in models:
            errors.append(
                f"ℹ️  Extra TypeScript interface: {interface_name} (no corresponding model)"
            )
...
def main():
    ...
    print(
        "🔍 Validating type sync between SQLAlchemy models and TypeScript interfaces..."
    )
