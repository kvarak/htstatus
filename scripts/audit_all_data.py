...
from models import db, Group, Match, MatchPlay, Players, PlayerSetting, User
...
def audit_all_data():
    ...
    with app.app_context():
        ...
        groups_by_user = (
            db.session.query(Group.user_id, db.func.count(Group.id))
        ...
        settings_by_user = (
            db.session.query(PlayerSetting.user_id, db.func.count(PlayerSetting.id))
            .group_by(PlayerSetting.user_id)
            .all()
        )
        ...
        if total_matches > 0:
            matches_by_team = (
                db.session.query(Match.home_team_id, db.func.count(Match.ht_id))
            ...
            for match in all_matches:
                print(
                    f"    Match {match.ht_id}: {match.home_team_name} vs {match.away_team_name} (Teams: {match.home_team_id} vs {match.away_team_id})"
                )
