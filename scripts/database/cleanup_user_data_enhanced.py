...
def cleanup_user_data(user_id, team_id):
    ...
    try:
        ...
        if settings_count > 0:
            cur.execute(
                "SELECT DISTINCT player_id FROM playersetting WHERE user_id = %s",
                (user_id,),
            )
            settings_player_ids = [row[0] for row in cur.fetchall()]

        # Combine all player IDs (deduplicated)
        all_related_player_ids = list(
            set(current_team_player_ids + settings_player_ids)
        )
        ...
        if all_related_player_ids:
            ...
            cur.execute(
                f"SELECT COUNT(*) FROM players WHERE ht_id IN ({placeholders})",
                all_related_player_ids,
            )
        ...
        if all_related_player_ids:
            ...
            placeholders = ",".join(["%s"] * len(all_related_player_ids))
            cur.execute(
                f"DELETE FROM matchplay WHERE player_id IN ({placeholders})",
                all_related_player_ids,
            )
        ...
        if all_related_player_ids:
            placeholders = ",".join(["%s"] * len(all_related_player_ids))
            cur.execute(
                f"DELETE FROM players WHERE ht_id IN ({placeholders})",
                all_related_player_ids,
            )
        ...
        if response == "yes":
            ...
            cur.execute(
                "SELECT COUNT(*) FROM playersetting WHERE user_id = %s", (user_id,)
            )
