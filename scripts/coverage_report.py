...
def get_coverage_analysis(quiet=False):
    ...
    try:
        ...
        if quiet:
            ...
            total_coverage = c.report(
                show_missing=False, skip_empty=True, file=sys.stdout
            )
        ...
        for filename in sorted(data.measured_files()):
            ...
            if any(
                pattern in filename for pattern in ["/tests/", "__pycache__", ".pyc"]
            ):
                continue
                    ...
                    file_stats.append(
                        {
                            "file": filename.replace(
                                "/Users/kvarak/repos/kvarak/htstatus-2.0/", ""
                            ),
...
def print_priority_targets(file_stats, limit=10):
    ...
    for _i, stats in enumerate(file_stats[:limit], 1):
        ...
        print(
            f"{file_short:<40} {stats['coverage']:6.1f}% {stats['missing_lines']:6d} {stats['impact_score']:6.1f}"
        )
...
def print_category_analysis(file_stats):
    ...
    categories = {"blueprints": [], "utils": [], "models": [], "chpp": [], "other": []}
    ...
    for category, files in categories.items():
        if files:
            ...
            print(
                f"{category.capitalize():<15}: {avg_coverage:6.1f}% avg, {total_missing:4d} missing lines"
            )
    ...
    if gap > 0:
        ...
        if high_impact:
            ...
            for f in high_impact[:3]:
                ...
                print(
                    f"   • {f['file'].split('/')[-1]}: Add ~{lines_to_cover} lines of tests"
                )

        # Look for blueprint routes (often easy to test)
        blueprint_files = [
            f for f in file_stats if "blueprints/" in f["file"] and f["coverage"] < 40
        ]
        if blueprint_files:
        ...
        utils_files = [
            f for f in file_stats if "utils" in f["file"] and f["missing_lines"] > 50
        ]
        if utils_files:
            print("\n🔧 UTILITY FUNCTIONS - High-value testing:")
            for f in utils_files[:2]:
                print(
                    f"   • {f['file'].split('/')[-1]}: {f['missing_lines']} uncovered lines"
                )
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate coverage analysis for test prioritization"
    )
    parser.add_argument(
        "--limit",
        "-l",
