...
class TestAuthBlueprintAdvanced:
    ...
    def test_login_get_request(self, mock_dprint, mock_create_page, mock_app, mock_db):
        ...
        with mock_app.test_request_context("/", method="GET"):
            ...
            mock_create_page.assert_called_once_with(
                template="login.html", title="Login / Signup"
            )
    ...
    def test_login_already_logged_in(
        self, mock_dprint, mock_redirect, mock_create_page, mock_app, mock_db
    ):
    ...
    @patch("app.blueprints.auth.dprint")
    def test_login_oauth_callback(
        self, mock_dprint, mock_handle_oauth, mock_app, mock_db
    ):
    ...
    def test_login_post_missing_username(
        self, mock_dprint, mock_create_page, mock_app, mock_db
    ):
        ...
        with mock_app.test_request_context(
            "/", method="POST", data={"password": "testpass", "cookie_consent": "on"}
        ):
            login()
    ...
    @patch("app.blueprints.auth.create_page")
    @patch("app.blueprints.auth.dprint")
    def test_login_post_missing_password(
        self, mock_dprint, mock_create_page, mock_app, mock_db
    ):
        ...
        with mock_app.test_request_context(
            "/", method="POST", data={"username": "testuser", "cookie_consent": "on"}
        ):
            login()
    ...
    @patch("app.blueprints.auth.create_page")
    @patch("app.blueprints.auth.dprint")
    def test_login_post_missing_cookie_consent(
        self, mock_dprint, mock_create_page, mock_app, mock_db
    ):
        ...
        with mock_app.test_request_context(
            "/", method="POST", data={"username": "testuser", "password": "testpass123"}
        ):
            login()
    ...
    @patch("app.blueprints.auth.create_page")
    @patch("app.blueprints.auth.dprint")
    def test_login_post_password_too_short(
        self, mock_dprint, mock_create_page, mock_user_model, mock_app, mock_db
    ):
    ...
    @patch("app.blueprints.auth.dprint")
    def test_login_post_existing_user_validation(
        self, mock_dprint, mock_check_password, mock_user_model, mock_app, mock_db
    ):
    ...
    @patch("app.blueprints.auth.dprint")
    def test_login_post_new_user_creation(
        self, mock_dprint, mock_generate_hash, mock_user_model, mock_app, mock_db
    ):
    ...
    @pytest.mark.skip(
        reason="Blueprints don't have url_map attribute until registered with app"
    )
    ...
    def test_login_debug_prints_coverage(self, mock_dprint, mock_app, mock_db):
        ...
        with (
            mock_app.test_request_context("/", method="GET"),
            patch("flask.session", {}),
            ...
            assert any(
                "LOGIN FUNCTION START" in str(call)
                for call in mock_dprint.call_args_list
            )
            assert any(
                "Showing login form" in str(call) for call in mock_dprint.call_args_list
            )

    @pytest.mark.skip(
        reason="Context manager protocol issues with mock_app and tuple unpacking"
    )
    @patch("app.blueprints.auth.dprint")
    def test_login_post_debug_coverage(self, mock_dprint, mock_app, mock_db):
        ...
        with (
            ...
            assert any(
                "Processing POST login request" in str(call)
                for call in mock_dprint.call_args_list
            )
            assert any(
                "Login attempt for username" in str(call)
                for call in mock_dprint.call_args_list
            )
            assert any(
                "Password provided" in str(call) for call in mock_dprint.call_args_list
            )
            assert any(
                "Cookie consent provided" in str(call)
                for call in mock_dprint.call_args_list
            )
