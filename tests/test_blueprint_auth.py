...
def auth_app():
    """Create application for authentication testing."""
...
class TestAuthBlueprintLoginRoutes:
    ...
    def test_login_post_short_password(self, client):
        ...
        response = client.post('/login', data={'username': 'testuser', 'password': '123'})
        assert response.status_code == 200
        assert b'Password must be at least 8 characters long' in response.data

    @patch('app.blueprints.auth.CHPP')
    def test_login_existing_user_with_tokens(self, mock_chpp_class, client, sample_user):
    ...
    def test_login_existing_user_without_tokens(self, mock_start_oauth, client, user_without_tokens):
...
class TestAuthBlueprintOAuthFlow:
    ...
    def test_start_oauth_flow_function(self, mock_render_template, mock_chpp_class, auth_app):
    ...
    def test_handle_oauth_callback_new_user(self, mock_user_class, mock_chpp_class, auth_app):
