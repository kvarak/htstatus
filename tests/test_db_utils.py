...
class TestParseDatabaseUrl:
    ...
    def test_parse_url_missing_auth(self):
        ...
        with pytest.raises(
            ValueError, match="Invalid DATABASE_URL format: missing auth"
        ):
            parse_database_url(url)
    ...
    def test_parse_url_missing_database(self):
        ...
        with pytest.raises(
            ValueError, match="Invalid DATABASE_URL format: missing database"
        ):
            parse_database_url(url)
...
class TestDatabaseUtilsIntegration:
    ...
    def test_end_to_end_database_config_loading(self, mock_load_dotenv):
        ...
        test_url = (
            "postgresql://htuser:securepass@proddb.example.com:5432/htstatus_production"
        )
