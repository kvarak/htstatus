...
class TestPlayerBlueprintRoutes:
    ...
    def test_player_route_no_team_id_uses_form(
        self, authenticated_client, sample_players
    ):
...
class TestPlayerGroupManagement:
    ...
    def test_update_player_group_assignment(self, client, db_session):
        ...
        setting = PlayerSetting.query.filter_by(
            player_id=player_ht_id, user_id=12345
        ).first()
    ...
    def test_remove_player_from_group(self, client, db_session):
        ...
        setting = PlayerSetting(
            player_id=player_ht_id, user_id=user.ht_id, group_id=group_id
        )
        ...
        setting = PlayerSetting.query.filter_by(
            player_id=player_ht_id, user_id=user.ht_id
        ).first()
    ...
    def test_update_existing_player_group(self, client, db_session):
        ...
        setting = PlayerSetting(
            player_id=player_ht_id, user_id=user.ht_id, group_id=group1_id
        )
        db_session.add(setting)
        db_session.commit()
...
class TestPlayerDataDisplay:
    ...
    def test_player_display_with_training_data(
        self, mock_get_training, authenticated_client, sample_players
    ):
