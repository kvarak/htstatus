...
def upgrade():
    ...
    result = connection.execute(
        sa.text(
            "SELECT ht_id, username, password FROM users WHERE password LIKE 'sha256$%'")
    )
    ...
    if sha256_users:
        print(
            f"Found {len(sha256_users)} users with SHA256 password hashes requiring migration")
        ...
        for user in sha256_users:
            connection.execute(
                sa.text(
                    "UPDATE users SET password = :new_password WHERE ht_id = :ht_id"),
                {
            ...
            print(
                f"Marked user {user.username} (ID: {user.ht_id}) for password migration")

        print(
            f"Migration complete. {len(sha256_users)} users will need to reset passwords.")
...
def downgrade():
    ...
    result = connection.execute(
        sa.text(
            "SELECT ht_id, username, password FROM users WHERE password LIKE 'MIGRATION_REQUIRED:sha256$%'")
    )
    ...
    if migrated_users:
        ...
        for user in migrated_users:
            ...
            original_password = user.password.replace(
                'MIGRATION_REQUIRED:', '')
            connection.execute(
                sa.text(
                    "UPDATE users SET password = :password WHERE ht_id = :ht_id"),
                {
            ...
            print(
                f"Restored SHA256 hash for user {user.username} (ID: {user.ht_id})")
